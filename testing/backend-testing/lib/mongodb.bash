...
function mongo_dump {
    ...
    mongodump --db=meetme --uri "$MONGO_URI" --out $temp_dir --excludeCollection=mongo_events 2> /dev/null && find $temp_dir -type f -print | xargs md5sum | cut -d ' ' -f1 | md5sum | cut -d ' ' -f1  || fail
