...
export default async function user(fastify, _opts) {
  ...
  fastify.route({
    ...
    schema: {
      ...
      params: S.object().prop("calendarId", S.string()).required(),
      body: S.object()
        .prop("timeblocks", S.array().items(timeblockSchema).minItems(1))
        .required()
        .prop("operation", S.string().enum(["ADD", "SUB", "SET"]))
        .required(),
      response: { 200: S.object().prop("calendar", Calendar.schema()) },
    },
    preHandler: validateTimeblocks,
  ...
  for (let i = 0; i < sorted.length - 1; ++i) {
    ...
    if (curr.start < next.end && next.start < curr.end) {
      throw new AppError.badRequest(
        `Timeblocks ${JSON.stringify(curr)} overlaps with Timeblocks ${JSON.stringify(next)}`,
      );
    }
  }
}
