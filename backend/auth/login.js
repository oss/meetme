...
router.post('/login',
    ...
    async function (req, res) {
        ...
        if (user === null) {
	    logger.info('creating new user', req, { uid: req.user.uid });
	    ...
	    logger.info('created new user', req, { uid: req.user.uid });
        }
        await update_last_login(req.user.uid);
	logger.info('user login', req, { user: req.user.uid });
...
router.get('/login',
    async function (req, res, next) {
	logger.info('started login session', req);
